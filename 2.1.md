...
> Baseline configuration of hosts to ensure that config setting match. 

# Standard naming conventions 
- Create a standard
  - Naming needs to be understood by everyone

- Exampples of things that need labeling
- Devices 
- Asset tag names
- computer names
- serial numbers
...
= An ip address plan or model Consistent addressing for network devices
  - Helps avoud duplicate IP addressing
...
- This causes Confusion 
  - The encrpyted data is drastically different than the plaintext 

- **Example** - Having the encrpytion longer than the text wrote

# Diffusion 
- Change one character of the input, and many characters change of the output. 
  - One character should change the cypher text
...
- Restrict data access to unauthroized persons
...
- SSL inspection relies on trust
  - SSL inspection allws you to be in the middle. 
...
# Hashing 
- Represnt data as a short string of text
  - A message digest 

- One-way trip
  - Impossible to recover the orginal message from the digest.

- Verify a download document that is the same as the orginal. 
...
- On-path attack 
  ...
  - replay API commands
  - Inject data into on API message. 
