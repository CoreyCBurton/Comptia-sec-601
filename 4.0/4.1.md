...
- use command ``-n`` to specify the number of lines 
...
- Evaluate the performace of security devices 

# DD
- A reference to the DD command in IBM mainfram JCL (Job Control Language)
...
- Copy to another host accross the network 
...
- Edit disk, files, RAM
  - Includes data reccovery features
...
- AccessData forensic drive imaging tool.
  - Include file utilites and read-only image mounting
  - Windows executable
...
- A pre-bulit toolkit for exploitations 
  - Bulid custom attacks
  - add more tools as vulnerabilites 
