...
- The protocol used to provide web content to browsers. HTTP uses port 80. HTTPS provides encrypted transfers ussing port 443.
...
- Tuneel mode
  - This mode is used for communications between VPN gateways across an unsecure netowrk creating a VPN
...
# Post Office Protocol v3 (POP3)
 - TCP port 110 that enables a client to access email messages stored in a mailbox on a remote server
 - THe server deletes messages once the client has downloaded them

# Internet Message Access Orticik v4 (IMAP4)
  - TCP/IP application protocol providing a means for a cleint to access and manage email messages stored in a mailbox on a remote server
    - supports permanet connections to a server over port 143 
